...
export default function Header({ setIsSidebarOpen, isSidebarOpen }: HeaderProps) {
  ...
  // Fetch all notifications when user changes or dialog opens
  useEffect(() => {
    const fetchNotifications = async () => {
      ...
      try {
        setLoading(true)
        // Fetch both general and schedule notifications
        const [generalData, scheduleData] = await Promise.all([
          api.getNotifications(user.id),
          api.getScheduleNotifications(user.id),
        ])
